# Copied from https://github.com/nerfstudio-project/nerfstudio/blob/main/nerfstudio/utils/tensor_dataclass.py
...
def td_concat(tds: List[TensorDataclassT]) -> TensorDataclassT:
    """
    concat a list of TensorDataclass, return a new TensorDataclass
    :param tds: a list of TensorDataclass
    :return: a new TensorDataclass with all fields concatenated
    """
    for td in tds:
        assert td.__class__ == tds[0].__class__, "td_concat only support same kind of TensorDataclass"
    data_dict: dict = {}
    for td in tds:
        for f in dataclasses.fields(td):
            data_dict.setdefault(f.name, [])
            data_dict[f.name].append(td.__getattribute__(f.name))
    for k, v in data_dict.items():
        if v[0] is None:
            data_dict[k] = None
        elif isinstance(v[0], torch.Tensor):
            data_dict[k] = torch.concat(v, dim=0)
        else:
            raise ValueError("td_concat only support TensorDataclass with torch.Tensor field")
    return tds[0].__class__(**data_dict)


if __name__ == '__main__':
    from camera.ray_utils import RayBundle
    ray_bundle1 = RayBundle(
    ...
    rb = td_concat([ray_bundle1, ray_bundle2])
    print(rb.shape)
